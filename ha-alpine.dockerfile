...
# Uses Home Assistant's Alpine base image with multi-arch support
ARG BUILD_FROM
FROM ${BUILD_FROM}
...
RUN mkdir -p /etc/supervisor.d /app/conf.d ${DEF_CUSTOM_ENTRYPOINTS_DIR} /var/log/supervisor
...
# Set the Home Assistant addon entrypoint
CMD ["/run.sh"]
